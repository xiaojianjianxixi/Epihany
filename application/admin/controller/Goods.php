...
class Goods extends Base {
    ...
    public function addEditCategory(){
            ...
            if(I('is_ajax') == 1)
            {
                ...
                {                          
                    ...
                    if(isset($_POST['commission_rate'])&&$GoodsCategory->commission_rate > 100)
                    {
                        //  编辑
                        $return_arr = array(
                            'status' => -1,
    ...
    {     
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            {                          
                ...
                $Goods->goods_image_url = $Goods->original_img;
    ...
    public function tastelist()
    {
        ...
        $tastelist = M('taste')->where(array('goods_id'=>$goods_id))->select();        
        $this->assign('goods_id',$goods_id);
        $this->assign('tastelist',$tastelist);
        return $this->fetch();       
    }
    
    public function tastelistadd()
    {
        $goods_id = $_GET['goods_id'];
        $this->assign('goods_id',$goods_id);
        return $this->fetch();
    ...
    public function tastelistadd_do()
    {
        $goods_id = $_POST['goods_id'];
        $tastename = trim(strip_tags($_POST['tastename']));

        if(empty($goods_id))
        {
            $return_arr = array(
                'status' => -1,
                'msg'   => 'no post goods_id',
                'data'  => '',
            );
            $this->ajaxReturn($return_arr);
            die();
    ...
    public function deltastelist()
    {
        $goods_id  = $_POST['goods_id'];
        $id = $_POST['id'];
    

        if(empty($goods_id) || empty($id))
        {
            $return_arr = array(
                'status' => -1,
        ...
        $result  = M('taste')->where(array('id'=>$id,'goods_id'=>$goods_id))->delete();
