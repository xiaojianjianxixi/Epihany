...
class Order extends Base {
    ...
    */
    ...
    public function deliveryHandle(){
		...
		if($res['flag']==1){
			...
			$this->error($res['message'],U('Admin/Order/delivery_info',array('order_id'=>$data['order_id'])));
		}
    }
    ...
    public function delivery_list(){
    	$condition['shipping_status'] = array('neq',1);
        $condition['order_status'] = array('in','1,2,4');
        $condition['pay_status'] = 1;
    	$delivery_count = M('order')->where($condition)->count();
    ...
    public function ajax_return_list(){
        ...
        empty($order_sn) && $status!=null && $where.= " and status = '$status' ";
        $count = M('return_goods')->where($where)->count();
        //dump(M('return_goods'));die;
    ...
    public function return_info()
    {
        ...
        $type_msg = array('退货退款','换货','仅退款');
        $status_msg = array('未处理','处理中','已完成');
        $return_status_msg = array('待审核','审核成功','审核失败');
        if(IS_POST)
        {
            $data['type'] = I('type');
            $return_status=I('return_status',0);
            $data['status'] = I('status');
            if($return_status==0){//待审核
        		//$data['status']=0;
        	}elseif($return_status==1){//成功
        		$data['status']=2;
        	}elseif($return_status==2){//失败
        		$data['status']=2;
        	}
        	$data['return_time']=time();
            $data['return_status']=$return_status;
            $data['remark'] = I('remark');                                    
            $note ="退换货:{$type_msg[$data['type']]}, 状态:{$status_msg[$data['status']]},审核结果:{$return_status_msg[$data['return_status']]},处理备注：{$data['remark']}";
            ...
            {        
            	...
            	if($data['status']==1){/*退款中*/
            		$update_order['return_status']=1;
            	}
            	if($return_status==1){
            		/*成功后*/
            		$update_order['return_status']=2;
            		/*修改商品信息已退货*/
            		$update_order['is_send']=3;
            	}elseif($return_status==2){
            		/*失败*/
            		$update_order['return_status']=3;
            	}
            	M('order_goods')->where($where)->save($update_order);//更改商品状态        
            	/*订单状态*/
            	if($return_status==1){
            		/*订单中全部退货成功则修改订单状态*/
            		$con['order_id']=$return_goods['order_id'];
            		$order_goods=M('OrderGoods')->where($con)->select();
            		$is_all=1;
            		...
            		if($is_all==1){
            			M('Order')->where('order_id='.$return_goods['order_id'])->save(array('order_status'=>6));
            		}
            	}
                $orderLogic = new OrderLogic();
   ...
   {                
    ...
    public function return_list(){
        ...
        $return_count = M('return_goods')->where($where)->count();
