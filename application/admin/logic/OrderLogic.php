...
class OrderLogic extends Model
{
    ...
    public function deliveryHandle($data){
		...
		foreach($orderGoods as $k=>$v){
			//退款中和退款中的不能发货
			if($v['return_status']==1||$v['return_status']==2){
				$result=array(
					'flag'=>-1,
					'message'=>'不能发货，此订单申请退货退款中'
				);
				return $result;
			}
		}
		...
		if($s && $r){
			...
			if($config==2){
				// 减少对应商品的库存
				minus_stock($data['order_id']);
			}
			$result=array('flag'=>1,'message'=>'操作成功');
		}else{
			$result=array('flag'=>-2,'message'=>'操作失败');
		}
		return $result;
    }
