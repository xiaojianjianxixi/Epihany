...
class Goods extends Base {
    ...
    */ 
    public function goodsInfo(){
        ...
        /*积分活动*/
       $user = session('user');
        if($goods['points_id'] > 0&&!empty($user)){
            $points_info=M('PointsGoods')->where('id='.$goods['points_id'])->find();
            if($points_info['group']){
            	$group=explode(',',$points_info['group']);
            	if(!in_array($user['level'],$group)){
            		/*不在促销范围内*/
            		$this->assign('points_no_flag',1);
            	}
            }
        }
        if ($user['user_id']) {
        ...
        $this->assign('sell_service',$config['sell_service']);   
        return $this->fetch();        
    }
    ...
    public function ajaxComment(){        
        ...
        foreach($list as $k => $v){
        	$user_str=explode(',',$v['zan_userid']);
        	if(!empty($user_str)&&cookie('user_id')>0){
        		if(in_array(cookie('user_id'),$user_str)){
        			$list[$k]['zan_class']='zan';
        		}else{
        			$list[$k]['zan_class']='';
        		}
        	}else{
    ...
    public function goodsConsult(){
        ...
        $username = I("username",'云商通用户'); // 网友咨询
    ...
    public function coupon(){
    	...
    	if($type=='coupon'){
    		/*优惠券*/
    		$this->assign('type','coupon');
    		$con['type']=5;//积分兑换
    		$con['send_start_time']=array('elt',time());
    		$con['send_end_time']=array('egt',time());
    		$count=M('coupon')->where($con)->count();
    		$page = new Page($count, 15);
    		$list=M('coupon')->where($con)->select();
	        $this->assign('page', $page->show());
    ...
    public function exchange_coupon(){
    	...
    	if($user_id == 0){
    		...
    		$coupon=M('coupon')->where('id='.$_POST['coupon_id'].' and type=5')->find();
    		if(empty($coupon)){
    			$data=array(
	    			'flag'=>-2,
	    			'message'=>'无效红包',
	    			'url'=>U('Home/Goods/coupon',array('type'=>'coupon'))
	    		);
    			exit(json_encode($data));
    		}
