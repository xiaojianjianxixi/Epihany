...
class User extends Base{
    ...
    public function points(){
        ...
        //获取账户资金记录
        $logic = new UsersLogic();
        $data = $logic->get_points_log($this->user_id,I('get.type'));
        $account_log = $data['result'];
        $this->assign('user',$user);
        $this->assign('account_log',$account_log);
        $this->assign('page',$data['show']);
        $this->assign('active','ppints');
        return $this->fetch();
    }
    /*
     * 优惠券列表
    ...
    public function login(){
        ...
        /*第三方登陆*/
        $Plugin=M('Plugin')->where("type='login' AND status = 1")->select();
...
//            $this->verifyHandle('user_reg');
    ...
    {          
            ...
            $add['service_rank'] = I('service_rank/d',5);
            $add['deliver_rank'] = I('deliver_rank/d',5);
            $add['goods_rank'] = I('goods_rank/d',5);
    ...
    */
    ...
    public function return_goods()
    {
	    ...
	    $spec_key = I('spec_key');
        $info = M('order')->where("order_id", $order_id)->where('user_id', $this->user_id)->find();
        if(empty($info))
        {
            $this->error('非法操作');
            exit;
        ...
        /*判断订单状态*/
        /*全部（订单处于其他状态）*/
        $return_goods = M('return_goods')->where(['order_id'=>$order_id,'goods_id'=>$goods_id,'spec_key'=>$spec_key])->order('addtime desc')->select();
        $this->assign('return_money',1);
        $this->assign('return_goods',0);
        if($info['order_status']==1&&$info['pay_status']==1&&$info['shipping_status']==1){/*待收货*/
    		$this->assign('return_goods',1);
    	}
        
        if(!empty($return_goods))
        {	
        	$this->assign('return_goods',$return_goods);
        	$count=count($return_goods);
        	/*申请只有一次*/
        	if($count>=1){
        		header("Location:".U('Home/User/return_goods_info',array('id'=>$return_goods[0]['id'])));
            	exit;
        	}else{
        		foreach($return_goods as $k=>$v){
	        		if($v['status']!=2){/*处理中和待审核*/
	        			header("Location:".U('Home/User/return_goods_info',array('id'=>$v['id'])));
	            		exit;
	        		}elseif($v['return_status']==1){/*已完成通过*/
	        			header("Location:".U('Home/User/return_goods_info',array('id'=>$v['id'])));
	            		exit;
	        		}
	        	}
        ...
        if(IS_POST)
        {
            ...
            $data['type'] = I('type'); // 服务类型  退款2，退货0 或者 换货1
            $data['reason'] = I('reason'); // 问题描述
            $data['imgs'] = I('imgs'); // 用户拍照的相片
            $data['spec_key'] = I('spec_key'); // 商品规格	
            $data['return_money']=I('return_money',0);
            M('return_goods')->add($data);        
            /*修改订单商品*/
            $update_con['order_id']=$order_id;
            $update_con['goods_id']=$goods_id;
            $update_con['spec_key']=I('spec_key'); // 商品规格	
            M('OrderGoods')->where($update_con)->save(array('return_status'=>1));/*退款中*/    
        ...
        $con['spec_key']=$spec_key;
        $goods=M('OrderGoods')->where($con)->find();
    ...
    {        
    	if(!empty($_REQUEST['search_key'])){
    		$where['order_sn']=array('like','%'.$_REQUEST['search_key'].'%');
    		$this->assign('search_key',$_REQUEST['search_key']);
    	}
        ...
        foreach($list as $k=>$v){
        	...
        	$list[$k]['goods_name']=M('OrderGoods')->where($con)->getField('goods_name');
        }
    ...
    public function return_goods_info()
    {
        ...
        /*历史记录*/
       	...
       	$con['order_id']=$return_goods['order_id'];
       	$con['goods_id']=$return_goods['goods_id'];
        $list=M('return_goods')->where($con)->order('addtime desc')->select();
        $this->assign('list',$list);
        $region_id[] = tpCache('shop_info.province');        
        $region_id[] = tpCache('shop_info.city');        
        $region_id[] = tpCache('shop_info.district');
    ...
    public function withdrawals(){
    	...
    	if(IS_POST)
    	{
    		...
    		$data = I('post.');
    		$data['user_id'] = $this->user_id;    		    		
    		$user_info=M('Users')->where('user_id='.$data['user_id'])->find();
    		if(empty($user_info)){
    			$this->error('无效操作');
                exit;
    		}
            ...
            $data['with_sn']='W'.date('YmdHis').rand(1000,9999);     
    		if(M('withdrawals')->add($data)){
    			accountLog($user_info['user_id'], ($data['money'] * -1), 0, "平台提现");
    			$this->success("已提交申请");
                        exit;
    		}else{
   ...
   public  function recharge(){
	    ...
	    if($_REQUEST['type']==1){
		   	...
		   	$show = $Page2->show();
		   	$consume_list = M('account_log')->where(array('user_id'=>$this->user_id,'user_money'=>array('lt',0)))->limit($Page2->firstRow.','.$Page2->listRows)->order('change_time desc')->select();
	        $this->assign('point_rate', tpCache('shopping.point_rate'));
		   	$this->assign('consume_list',$consume_list);//消费记录(支出)
		   	$this->assign('page2',$show);
	    }else{
	    	$count2 = M('account_log')->where(array('user_id'=>$this->user_id,'user_money'=>array('gt',0)))->count();
		   	$Page2 = new Page($count2,10);
		   	$show = $Page2->show();
		   	$consume_list = M('account_log')->where(array('user_id'=>$this->user_id,'user_money'=>array('gt',0)))->limit($Page2->firstRow.','.$Page2->listRows)->order('change_time desc')->select();
	        $this->assign('point_rate', tpCache('shopping.point_rate'));
		   	$this->assign('consume_list',$consume_list);//消费记录(收入)
		   	$this->assign('page',$show);
	    	/*$count = M('recharge')->where(array('user_id'=>$this->user_id))->count();
		   	$Page = new Page($count,10);
		   	$show = $Page->show();
		   	$recharge_list = M('recharge')->where(array('user_id'=>$this->user_id))->limit($Page->firstRow.','.$Page->listRows)->order('ctime desc')->select();
		   	$this->assign('page',$show);
    ...
    public function message_notice()
    {
        ...
        ->select();
    	//$user_logic->setSysMessageForRead();//更新系统消息为已读
        $this->assign('messages', $user_system_message);
        $this->assign('page', $page->show());
    	$this->assign('active','message_notice');
    ...
    public function ajax_message_notice()
    {
        ...
        $message_model = new Message();
        //系统消息
        $user_sys_message = $message_model->getUserMessageNotice();//插入系统消息
        $user_logic->setSysMessageForRead();//更新系统消息为已读
       /* if ($type == 1) {
            //系统消息
