...
class CartLogic extends Model
{
     ...
     * @param type $user_id 用户id
     */
    function addCartBuy($goods_id,$goods_num,$goods_spec,$session_id,$user_id = 0)
    {       
        $goods = M('Goods')->where("goods_id", $goods_id)->cache(true,TPSHOP_CACHE_TIME)->find(); // 找出这个商品
        $specGoodsPriceList = M('SpecGoodsPrice')->where("goods_id", $goods_id)->cache(true,TPSHOP_CACHE_TIME)->getField("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码
	    $where = " session_id = :session_id ";
        ...
        if($goods['prom_type'] == 1) 
        {
            $flash_sale = M('flash_sale')->where(['id'=>$goods['prom_id'],'start_time'=>['<',time()],'end_time'=>['>',time()],'goods_num'=>['>','buy_num']])->find(); // 限时抢购活动
            if($flash_sale){
                $cart_goods_num = M('CartBuy')->where($where)->where("goods_id", $goods['goods_id'])->bind($bind)->getField('goods_num');
                // 如果购买数量 大于每人限购数量
                if(($goods_num + $cart_goods_num) > $flash_sale['buy_limit'])
                {  
                    $cart_goods_num && $error_msg = "你当前购物车已有 $cart_goods_num 件!";
        ...
        $catr_goods = M('CartBuy')->where($where)->bind($cart_bind)->find(); // 查找购物车是否已经存在该商品
        $price = $spec_price ? $spec_price : $goods['shop_price']; // 如果商品规格没有指定价格则用商品原始价格
       ...
       {          
           // 如果购物车的已有数量加上 这次要购买的数量  大于  库存输  则不再增加数量
            if(($catr_goods['goods_num'] + $goods_num) > $goods['store_count'])
                $goods_num = 0;
            $result = M('CartBuy')->where("id", $catr_goods['id'])->save(  array("goods_num"=> ($catr_goods['goods_num'] + $goods_num)) ); // 数量相加
            return array('status'=>1,'msg'=>'成功加入购物车');
       }
       else
       {         
             $insert_id = M('CartBuy')->add($data);
             return array('status'=>1,'msg'=>'成功加入购物车','cart_id'=>$insert_id);
       }     
    ...
    {                   
        ...
        $cartList = M('Cart')->where($where)->order('id desc')->bind($bind)->select();  // 获取购物车商品
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    function cartListbuy($user = array() , $session_id = '', $selected = 0,$mode =0)
    {                   
        ...
        $cartList = M('CartBuy')->where($where)->order('id desc')->bind($bind)->select();  // 获取购物车商品
        $anum = $total_price =  $cut_fee = 0;
        foreach ($cartList as $k=>$val){
        	$cartList[$k]['goods_fee'] = $val['goods_num'] * $val['member_goods_price'];
        	$cartList[$k]['store_count']  = getGoodNum($val['goods_id'],$val['spec_key']); // 最多可购买的库存数量        	
     ...
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='',$invoice_tax='',$invoice_type=0)
    {
        if($invoice_type==1){
        	if(empty($invoice_tax)||empty($invoice_title)){
        		return array('status'=>-10,'msg'=>'请填写发票抬头和税号','result'=>'');      
        	}
        }
        ...
        $data['integral']=ceil($data['integral']/1);//有小数点自动加一
        ...
        if($car_price['pointsFee']>0)
        	M('Users')->where("user_id", $user_id)->setDec('pay_points',$data['integral']); // 消费积分
        ...
        $data4['pay_points'] = -$data['integral'];
     ...
     * @return type $order_id 返回新增的订单id
     */
    public function addOrderBuy($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='',$invoice_tax='',$invoice_type=0)
    {
        if($invoice_type==1){
        	if(empty($invoice_tax)||empty($invoice_title)){
        		return array('status'=>-10,'msg'=>'请填写发票抬头和税号','result'=>'');      
        ...
        // 1插入order_goods 表
        $cartList = M('CartBuy')->where(['user_id'=>$user_id,'selected'=>1])->select();
        foreach($cartList as $key => $val)
        {
           $goods = M('goods')->where("goods_id", $val['goods_id'])->cache(true,TPSHOP_CACHE_TIME)->find();
           $data2['order_id']           = $order_id; // 订单id
        ...
        if($car_price['balance']>0)
        	M('Users')->where("user_id", $user_id)->setDec('user_money',$car_price['balance']); // 抵扣余额
        // 4 删除已提交订单商品
        M('CartBuy')->where(['user_id' => $user_id,'selected' => 1])->delete();
   ...
   public function login_cart_handle($session_id,$user_id)
   {
	   if(empty($session_id) || empty($user_id))
        ...
        // 登录后将购物车的商品的 user_id 改为当前登录的id （立即购买）       
        M('CartBuy')->where("session_id", $session_id)->save(array('user_id'=>$user_id));
                
        // 查找购物车两件完全相同的商品（立即购买）
        $cart_id_arr = DB::query("select id from `__PREFIX__cart_buy` where user_id = $user_id group by  goods_id,spec_key having count(goods_id) > 1");
        if(!empty($cart_id_arr))
        {
            $cart_id_arr = get_arr_column($cart_id_arr, 'id');
            $cart_id_str = implode(',', $cart_id_arr);
            M('CartBuy')->delete($cart_id_str); // 删除购物车完全相同的商品
        }
   }
    /**
     * 添加预售商品订单
