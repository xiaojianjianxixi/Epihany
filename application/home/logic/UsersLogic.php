...
class UsersLogic extends Model
{
     ...
     * 第三方绑定
     */
    public function thirdBind($data=array()){
        $unionid = $data['unionid']; //第三方返回唯一标识
        $user_id = $data['user_id'];
        $oauth = $data['oauth']; //来源(默认微信)
        $oauth = !empty($oauth)?$oauth:'wx';
        if(!$unionid || !$oauth)
            return array('status'=>-1,'msg'=>'参数有误','result'=>'');
        //获取用户信息
        $con[$oauth.'_unionid']=$unionid;
        $user=M('Users')->where($con)->find();
        if(!$user){
            //可以绑定
            $map[$oauth.'_unionid'] = $unionid;
           	$res=M('Users')->where('user_id='.$user_id)->save($map);
           	if($res!==false){
           		return array('status'=>1,'msg'=>'绑定成功','result'=>'');
           	}else{
           		return array('status'=>-3,'msg'=>'绑定失败','result'=>'');
           	}
        }else{
            return array('status'=>-2,'msg'=>'该账号已被绑定','result'=>'');
        }
...
//        $type = I('get.type',0);
     ...
     * 获取账户积分记录
     */
    public function get_points_log($user_id,$type=0){
        //查询条件
        if($type == 1){
            //收入
            $where = 'pay_points > 0 AND user_id=:user_id';
        }
        if($type == 2){
            //支出
            $where = 'pay_points < 0 AND user_id=:user_id';
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
        	...
        	left join __PREFIX__order as o on o.order_id = og.order_id where  o.order_status in(2,4)and o.user_id = $user_id  and og.is_send = 1 ";
        	$where = '';
        	if($status == 0){
    ...
    public function cancel_order($user_id,$order_id){
		...
		if($order['coupon_price']>0){
			$coupon_con['uid']=$order['user_id'];
			$coupon_con['order_id']=$order['order_id'];
			$set['order_id']='0';
			$set['use_time']='0';
			M('CouponList')->where($coupon_con)->save($set);
		}
    ...
    public function check_validate_code($code, $sender, $type ='email', $session_id=0 ,$scene = -1){
        ...
        if($type == 'email'){            
            ...
            $sms_time_out = tpCache('sms.sms_time_out');
            $sms_time_out = $sms_time_out ? $sms_time_out : 180;
            $con=array('mobile'=>$sender , 'status'=>1);
            if($session_id!=0){
