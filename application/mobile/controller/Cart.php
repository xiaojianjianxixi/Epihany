...
class Cart extends MobileBase {
    ...
    public function addCart()
    {
        ...
        $goods_id = $data[0]['id']; // 商品id
        $goods_num = $data[0]['num'];// 商品数量
    ...
    public function ajaxAddCart()
    {
        ...
        // {
        //     $sessioncart  = session('hybldcart');

        //     foreach ($data['goods'] as $key =>$value)
        //     {
        //         if(empty($sessioncart[$value['goods_id']])){
        //             $sessioncart[$value['goods_id']]['goods_id'] = $value['goods_id'];
        //             $sessioncart[$value['goods_id']]['goods_num'] = $value['goods_num']; 
        //             $sessioncart[$value['goods_id']]['session_id'] = $session_id; 
        //         }else{
        //             $sessioncart[$value['goods_id']]['goods_num'] = intval($value['goods_id']) + intval($sessioncart[$value['goods_id']]['goods_num']);
        //         }
        //     }
         
        //     if($sessioncart)
        //     {   
        //         $_SESSION['hybldcart'] = $sessioncart;
         
        //         if(session('hybldcart')){
        ...
        //         }
        //     }
        //     //session 添加购物车后转为 读取购物车页面的时候必须先读session 
    ...
    /**
     *  没登录将数据购物车数据添加到session
     */
    public function loginFillcart($goods_id,$goods_num)
    {
       $user = session('user');
       if(empty($user))
       {
     ...
     * ajax 将商品加入购物车
     */
    function ajaxAddCartBuy()
    {
    	if($this->user_id != 0){
        	/*清空购物车*/
    		M('CartBuy')->where('user_id='.$this->user_id)->delete();
        }
        $goods_id = I("goods_id/d"); // 商品id
        $goods_num = I("goods_num/d");// 商品数量
        $goods_spec = I("goods_spec/a",array()); // 商品规格
        $result = $this->cartLogic->addCartBuy($goods_id, $goods_num, $goods_spec,$this->session_id,$this->user_id); // 将商品加入购物车
        exit(json_encode($result));
    }
    /*
     * 请求获取购物车列表
    ...
    public function cart2()
    {
        ...
        if(empty($address)){
        	...
        	$address['province_name']=M('Region')->where('id='.$address['province'])->getField('name');
       		$address['city_name']=M('Region')->where('id='.$address['city'])->getField('name');
        	$address['district_name']=M('Region')->where('id='.$address['district'])->getField('name');
        ...
        if($type=='buy'){
        	//立即购买
			$count = M('CartBuy')->where(['user_id' =>$this->user_id, 'selected' => 1])->count();
        	if($count<=0){
	        	$this->error ('你的购物车没有选中商品','Cart/cart');
	        }
        	$result = $this->cartLogic->cartListbuy($this->user,$this->session_id,1,1); // 获取购物车商品
        }else{
    ...
    public function cart3(){
        ...
        $invoice_type = I('invoice_type',0); // 发票类型  0个人  1企业
		...
		if($type=='buy'){
	        ...
	        $order_goods = M('CartBuy')->where(['user_id'=>$this->user_id,'selected'=>1])->select();
		}else{
	...
	// 订单满额优惠活动		                
        ...
        {  
            ...
            if($type=='buy'){
            	$result = $this->cartLogic->addOrderBuy($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$invoice_tax,$invoice_type); // 添加订单   
            }else{
            	$result = $this->cartLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$invoice_tax,$invoice_type); // 添加订单     
            }
            exit(json_encode($result));
        }
    ...
    */
    public function ajaxCartList()
    {
        ...
        //     $result['cartList'][$key]['goods_image_url']  = M('Goods')->where(array('goods_id'=>$value['goods_id']))->getField('goods_image_url');        
        // }
        ...
        $this->assign('cartList', $result['cartList']); // 购物车的商品    
        $this->assign('count', $count); // 购物车的商品                
        $this->assign('total_price', $result['total_price']); // 总计
