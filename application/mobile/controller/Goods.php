...
class Goods extends MobileBase {
    ...
    public function goodsList(){
    	/*row横向   column纵向*/
    	$this->assign('listtype',I('listtype','column'));
    ...
    public function ajaxGoodsList() {
        ...
        if($cat_id > 0)
        {
        	...
        	$where .= " WHERE goods_name like '%".$keyword."%'"; // 初始化搜索条件
        }
    	...
    	/*销量*/
    	$sales=I('salessort/d',0);
    	$price=I('pricesort/d',0);
    	$type=I('type');
    	if($sales>0){
    		if($sales==1){
        ...
        //echo "select *  from __PREFIX__goods $where $order $limit";die;
...
//        $collect = M('goods_collect')->where("goods_id=$goods_id and user_id=$user_id")->count();
	...
	public function integralcoupon(){
        ...
        $goods_list = M('goods')->where($goods_where)->limit(4)->select();
        $this->assign('goods_list', $goods_list);
        $this->assign('point_rate',$point_rate);//总页数
        /*优惠券*/
	  	$where=array();
	  	$where['type']=5;
	...
	public function coupon(){
		/*优惠券*/
	  	...
	  	$where['send_start_time']=array('elt',time());
        $where['send_end_time']=array('egt',time());
        $coupon_list_count = M('Coupon')->where($where)->count();   //总页数
        $page = new Page($coupon_list_count, 16);
        $coupon_list=M('Coupon')->where($where)->field('id,name,money,condition,points')->limit($page->firstRow . ',' . $page->listRows)->order('add_time desc')->select();
        $this->assign('coupon_list',$coupon_list);
        if(IS_AJAX){
            return $this->fetch('ajaxCoupon'); //获取更多
        }
        return $this->fetch();
	}
	public function getcoupon(){
		...
		$where['id']=I('coupon_id');
		$where['type']=5;
	  	$where['send_start_time']=array('elt',time());
        $where['send_end_time']=array('egt',time());
		$coupon_info=M('Coupon')->where($where)->find();
		...
		$remain = $coupon_info['createnum'] - $coupon_info['send_num'];//剩余派发量
		if($remain<=0){
			$data=array(
    			'status'=>-4,
    			'message'=>'优惠券被领光了',
		...
		if($coupon_info['points']>$user_info['pay_points']){
			$data=array(
				'status'=>-1,
				'msg'=>'您的积分不足'
			);
		...
		if(I('act')=='submit_get'){
			/*扣除会员积分并插入记录*/
    		accountLog($user_id, 0, ($coupon_info['points'] * -1), "兑换优惠券");
    		/*插入优惠券记录*/
    		$insert_coupon=array(
    			'cid'      => $coupon_info['id'],
    			'type'     => 5,
    		...
    		/*更新优惠券信息*/
    		$update_coupon['send_num']=$coupon_info['send_num']-1;
    		$update_coupon['id']=$coupon_info['id'];
    ...
    {       
        ...
        if($dispatching_data['status']==1){
			exit(json_encode($dispatching_data)); 
        }else{
			exit(json_encode($dispatching_data)); 
        }
    }
