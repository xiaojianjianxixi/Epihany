...
class User extends MobileBase
{
    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            'login', 'pop_login', 'do_login','do_login_no_verify', 'logout', 'verify', 'set_pwd', 'finished',
            'verifyHandle', 'reg', 'send_sms_reg_code', 'find_pwd','ajaxforget_pwd', 'check_validate_code',
    ...
    public function index()
    {
    	...
    	$goods_collect_count=M('goods_collect')->alias('gc')
            ...
            ->where("gc.user_id=".$this->user_id.' and g.goods_id >0')
            ->count();
        ...
        $comment = DB::query("select COUNT(1) as comment from __PREFIX__order_goods as og left join __PREFIX__order as o on o.order_id = og.order_id where o.order_status in(2,4) and o.user_id = $this->user_id and og.is_send = 1 and og.is_comment = 0 ");  //我的待评论订单
...
//            header("Location: " . U('Mobile/User/index'));
     ...
     * 登录没有图形验证码验证
     */
    public function do_login_no_verify()
    {
        $username = I('post.username');
        $password = I('post.password');
        $username = trim($username);
    ...
    /*查看物流*/
 	public function shippping_code()
    {
        $order_id = I('get.order_id/d', 195);
        $order_goods = M('order_goods')->where("order_id", $order_id)->select();
        $delivery = M('delivery_doc')->where("order_id", $order_id)->find();
        
        $url='http://m.kuaidi100.com/index_all.html?type='.$delivery['shipping_name'].'&postid='.$delivery['invoice_no'].'&callbackurl='.$_SERVER['HTTP_REFERER'];
    	header("Location: ".$url);
    	exit(); 
    }
    /*
     ...
     * 设置默认收货地址
     */
    public function ajax_et_default()
    {
        $id = I('id/d');
        M('user_address')->where(array('user_id' => $this->user_id))->save(array('is_default' => 0));
        $row = M('user_address')->where(array('user_id' => $this->user_id, 'address_id' => $id))->save(array('is_default' => 1));
...
/*
    ...
    public function saveuserinfo()
    {
        ...
        if (!empty($mobile)) {
            ...
            if($c){
            	$msg['ReturnCode'] = -1;
				$msg['ReturnMsg'] = '手机已被使用';
		        exit(json_encode($msg));
            };
            if($this->user['mobile']!=$mobile){
            	$msg['ReturnCode'] = -5;
				$msg['ReturnMsg'] = '手机号不匹配';
		        exit(json_encode($msg));
            }
            if (!$code){
            	$msg['ReturnCode'] = -2;
				$msg['ReturnMsg'] = '请输入验证码';
		        exit(json_encode($msg));
            }
            $check_code = $userLogic->check_validate_code($code, $mobile, 'phone', $this->session_id, $scene);
            if ($check_code['status'] != 1){
    ...
    public function order_confirm()
    {
        ...
        if (!$data['status']) {
        	...
        	if(isset($_REQUEST['type'])){
        		$url=U('User/order_list',array('type'=>$_REQUEST['type']));
        	}else{
        		$url=U('User/order_detail',array('id'=>$id));
        	}
        	$this->success('确认收货成功',$url);
            /*$model = new UsersLogic();
...
//        $goods = M('goods')->where("goods_id", $goods_id)->find();
     ...
     * 提交申请退货（修改）
     */
    public function return_goods_money()
    {
        $order_id = I('order_id/d', 0);
        $order_sn = I('order_sn', 0);
        $goods_id = I('goods_id/d', 0);
        ...
        if (IS_POST) {
            ...
            $data['addtime'] = time();
            $data['user_id'] = $this->user_id;
            $data['type'] = I('type'); // 0仅退款  1换货   2退货退款
            $data['return_money']=I('return_money/d',0);
            $data['reason'] = I('reason'); // 问题描述     
            $data['spec_key'] = I('spec_key'); // 商品规格						       
            $res = M('return_goods')->add($data);
            $data['return_id'] = $res;  //退换货id
...
//            $this->success('申请成功,客服第一时间会帮你处理', U('Mobile/User/order_list'));
        ...
        return $this->fetch('return_goods');
    }
    /**
     * 退换货列表
     */
    ...
    public function set_head_img(){
    	...
    	$base64=$_POST['image'];
		$url = explode(',',$base64);
		$dir = 'public/upload/head_pic/' . date('Y', time()) . '/' . date('m-d', time()).'/';
		if(!is_dir($path)){
			mk_dir($dir,0777,true);
		}
		
		$savename=substr(md5(GetStr(8)),20);
		while(file_exists($dir.$savename.'.png')){
			$savename=substr(md5(GetStr(8)),20);
		}
		$a = file_put_contents($dir.$savename.'.png', base64_decode($url[1]));//返回的是字节数
		if($a>0){
			$thumb='/'.$dir.$savename.'.png';
			if(!empty($thumb)){
	        	$post['head_pic']=$thumb;
	        }
	        if (!$userLogic->update_info($this->user_id, $post)){
