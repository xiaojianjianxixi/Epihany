$(function () {
    ...
    function addGoods (num) {
      ...
      var good = goods.find(function (item) {
        return item.id == currentGood.id
      })
      if(good) {
        good.num += num
      } else {
        currentGood.num = num
        goods.push(currentGood)
      }
    }
    ...
    $(".add").click(function(){
        ...
        var tastename=parent.parent().children("span").text()
        var price = parseFloat(parent.prev().children("b:nth-child(2)").text());
        var src = $(this).parents('li').find('.shop_icon').attr('src')
        var id = $(this).parents('.good-item').find('.good-id').val()
        console.log(name,price,src,tastename);
        currentGood = {
          name: name,
          price: price,
          id: id,
        ...
        $(".pce").text(price);
        $(".m-active").text(tastename);
        $(".imgPhoto").attr('src',src);
        $(".goods_id_s").val(id);
        $("#totalpriceshow").text(price);
 ...
 //       $('.shopcart-list').show();
    ...
    $(".foot_s").click(function () {
        ...
        var goods_id = $(".goods_id_s").val();
        console.log('goods_id:'+goods_id)

        addGoods(n)
        
        // var oldnum = $("#totalcountshowold").text();
       

        // var oldprice = $("#totalpriceshowold").text();

        // var allnum = Number(n) + Number(oldnum);
        var total = countGoods()

        console.log(total)

        $("#totalcountshow").text(total.totalcount);
  
        // var allprice = Number(n) * parseFloat(danjia) + parseFloat(oldprice); 
        // var a = $("#totalpriceshow").html();//获取当前所选总价
        $("#totalpriceshow").text(total.totalprice);

        // if(n==oldnum){
        //     $("#totalpriceshow").html(a);//计算当前所选总价
        // }else{
        //     $("#totalpriceshow").html((a * 1 + danjia * 1).toFixed(2));//计算当前所选总价
        // }
        //  $("#totalpriceshow").html(allprice);//计算当前所选总价
       //var nm = $("#totalcountshow").html(total.totalcount);//获取数量
       // $("#totalcountshow").html(nm*1+1);
        //$("#totalcountshow").html(total.totalprice);
        jss();//  改变按钮样式
        $(".subFly").hide();
        var ms = e.text(num-1);
        if(ms!=0){
    ...
    //     var n = parseInt($('.ad').prev().text());
    //     var num = parseFloat(n) + 1;
    //     console.log('n:'+n)

    //     var dpjq =n*parseInt($('.pce').text())
    //     console.log(dpjq)

    //     var oldnum = parseInt($("#totalcountshow").html());
    //     var oldjq =parseInt($('.totalpriceshow').html())
    //     console.log('oldnum:'+oldnum)
    //     console.log('oldjq:'+oldjq)
    //     var newnum =oldnum+n;
    //     var newjq =oldjq+dpjq;
    //     $("#totalcountshow").text(newnum);
    //     $('.totalpriceshow').text(newjq);
    //     newnum=0;newjq=0
    //     jss();//  改变按钮样式
    //     $(".subFly").hide();
    //     var ms = e.text(num-1);
