<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:38:"./template/pc/rainbow/index/index.html";i:1523930735;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
            ...
            <div class="w1224 pr p">
                ...
                <div class="top-ri-header fr">
                    <ul>
                        ...
                        <li class="spacer"></li>
                        <li class="hover-ba-navdh" style="display: none;"> 
                            <div class="nav-dh">
                                <span>网站导航</span>
                    ...
                    <div class="u-g-cart fr fixed" id="hd-my-cart">
                        <a href="<?php echo U('Home/Cart/cart'); ?>">
                            <div class="c-n fl" >
                                ...
                                <span>我的购物车</span>
                                <div  style="color:#e23435;display:inline-block;" >(<span class="count cart_quantity"   id="cart_quantity">0</span>)</div>
                            </div>
                        </a>
        ...
        <div class="nav p">
            <div class="w1224 p">
                <div class="categorys home_categorys">
                    <div class="dt">
                        <a href="javascript:;"><i class="share-a_a2"></i>全部商品分类</a>
                    </div>
                    <!--全部商品分类-s-->
                    <div class="dd">
                        <div class="cata-nav">
                            <!-- 外层循环点-->
                            <?php if(is_array($goods_category_tree) || $goods_category_tree instanceof \think\Collection): if( count($goods_category_tree)==0 ) : echo "" ;else: foreach($goods_category_tree as $k=>$v): ?>
                                    ...
                                    <div class="cata-nav-left">
                                        <div class="subitems">
                                            ...
                                            <div class="advertisement_down">
                                                <ul>                                                
                                                    <?php $pid =14;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
...
foreach($result as $key=>$v3):       
    ...
    if(I("get.edit_ad") && $v3[not_adv] == 0 )
    {
    	...
    	$v3[style] = "background:$v3[bgcolor]"; // 广告背景图样式
        $v3[title] = $v3[ad_name];
    }	
    ?>
                                                        <li>
                                    ...
                                    <div class="cata-nav-rigth">
                                        <?php $pid =51;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    if(I("get.edit_ad") && $v[not_adv] == 0 )
    {
    	...
    	$v[style] = "background:$v[bgcolor]"; // 广告背景图样式
        $v[title] = $v[ad_name];
    }	
    ?>
    ...
    <div id="myCarousel" class="carousel slide p header-tp" data-ride="carousel">
        ...
        <div class="carousel-inner">
        	<?php $adk = '1'; $pid =2;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v1):       
    ...
    if(I("get.edit_ad") && $v1[not_adv] == 0 )
    {
    	...
    	$v1[style] = "background:$v1[bgcolor]"; // 广告背景图样式
        $v1[title] = $v1[ad_name];
    }	
    ?>
        ...
        <div class="adcertiserment_head">
            <div class="w1224">
                <ul>
                    <?php $pid =52;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("2")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    <div class="adv3 p">
        <div class="w1224">
            <ul>
                <?php $pid =53;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("3")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    <div class="adver_line">
        <div class="w1224">
            <?php $pid =3;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
<!--分类广告-->
<!--------楼层-开始-------------->
    <?php if(is_array($cateList) || $cateList instanceof \think\Collection): if( count($cateList)==0 ) : echo "" ;else: foreach($cateList as $k=>$v): ?>
    <!--商品楼层-s-->
        <div class="layer-floor " id="floor<?php echo $k+1; ?>">
            ...
            <div class="top_title_layer p">
                ...
                <div class="part-hot">
                    <ul>
                        <?php if(is_array($v[tmenu]) || $v[tmenu] instanceof \think\Collection): if( count($v[tmenu])==0 ) : echo "" ;else: foreach($v[tmenu] as $k2=>$v2): ?>
                            <li>
            ...
            <div class="main_layer p">
                <div class="hoste_le">
                	<?php $tmp=$k+1; $pid =51317;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1524020400 and end_time > 1524020400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("$tmp")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
$c = $tmp- count($result); //  如果要求数量 和实际数量不一样 并且编辑模式
if($c > 0 && I("get.edit_ad"))
{
    for($i = 0; $i < $c; $i++) // 还没有添加广告的时候
    {
...
foreach($result as $cat=>$vo):       
    
    $vo[position] = $ad_position[$vo[pid]]; 
    if(I("get.edit_ad") && $vo[not_adv] == 0 )
    {
        $vo[style] = "filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity: 0.5"; // 广告半透明的样式
        $vo[ad_link] = "/index.php?m=Admin&c=Ad&a=ad&act=edit&ad_id=$vo[ad_id]";        
        $vo[title] = $ad_position[$vo[pid]][position_name]."===".$vo[ad_name];
        $vo[target] = 0;
    }else{
    	$vo[style] = "background:$vo[bgcolor]"; // 广告背景图样式
        $vo[title] = $vo[ad_name];
    }	
    if($k == $cat): ?>
	                    	<a class="big_adve" href="<?php echo $vo['ad_link']; ?>" <?php if($vo['target'] == 1): ?>target="_blank"<?php endif; ?>>
		                    <img src="<?php echo $vo['ad_code']; ?>" width="232" height="571"  title="<?php echo $vo[title]; ?>" style="<?php echo $vo[style]; ?>"/>
		                    </a>
	                    <?php endif; endforeach; ?>
                </div>
                <div class="hoste_ri">
                    <ul>
                        <?php if(is_array($v[hot_goods]) || $v[hot_goods] instanceof \think\Collection): if( count($v[hot_goods])==0 ) : echo "" ;else: foreach($v[hot_goods] as $gk=>$g): if($gk < 8): ?>
                                <li>
                                    <a href="<?php echo U('Home/Goods/goodsInfo',array('id'=>$g[goods_id])); ?>">
                                        ...
                                        <span class="name_main"><?php echo getSubstr($g[goods_name],0,13); ?></span>
        ...
        <ul>
            ...
            <li class="re_wechat">
                ...
                <div class="rtipscont" style="">
                    ...
                    <img src="<?php echo $tpshop_config['shop_info_store_weixin']; ?>"/>
